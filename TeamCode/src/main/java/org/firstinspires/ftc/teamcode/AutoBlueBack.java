...
// AUTO BLUE BACK    AUTONOMOUS BY TIME WITH COLOR, DISTANCE and TOUCH CODE
//
// CHANGES
//
...
@Autonomous(name = "AutoBlueBack", group = "Sensor")
...
public class AutoBlueBack extends LinearOpMode {
    ...
    //BELOW VALUES IN SECONDS
            //double CloseArms            = 1.5;      	//Step 1
    double DropColorArm         = 1.0;
    double ReadColor            = 1.0;

    double GlyphMotorPowerUp 	=  .5;     	//RAISE GLYPH ARM
    double GlyphMotorPowerDown	= -.2;	    	//LOWER GLYPH ARM, BUT NOT COMPLETELY .3

    double KnockOffBall         = 1.5;      	//
    //double KnockOffSpeedNeg	= -.25;	    	//REMOVED KNOCK OFF ROTATE SPEED NEG
    //double KnockOffSpeedPos	=  .25;	    	//REMOVED KNOCK OFF ROTATE SPEED POS

    double ResetMove            = 1.0;      	//
    //double ResetSpeedNeg		= -.22;	    		//REMOVED RESET ROTATE SPEED NEG
    //double ResetSpeedPos		=  .22;	    		//REMOVED RESET ROTATE SPEED POS
    ...
    int MoveOffBoardClicksLeft  = -1000;     	//Backward
    int MoveOffBoardClicksRight =  1000;    	//Backward
    double MOBPower 		    = .3;

    double Spin                 = 3.5;		//
    int Spin90left              = -1020; 	//Spin CounterClockwise  -970 -1000 1010
    int Spin90right             = -1020;		// -970
    double SpinPower 		    = .7;


    //MECANUM CHASSIS - Move Parallel to Cryptobox
    int MTGClicksLeftLEFT      =  142;     	//LEFT - Forward Right 230 107 66
    int MTGClicksRightLEFT     = -142;     	//LEFT - Forward Right 230 107
    int MTGClicksLeftCENTER    =  476;     	//CENTER - Forward Right was 486
    int MTGClicksRightCENTER   = -476;     	//CENTER - Forward Right
    int MTGClicksLeftRIGHT     =  832;     	//RIGHT - Forward Right 759
    int MTGClicksRightRIGHT    = -832;     	//RIGHT - Forward Right
    double MTGPower 		   = .3;


    //STATE VALUES ORIG CHASSIS
    //int MTGClicksLeftLEFT      = -230;     	//LEFT - Forward Right
    //int MTGClicksRightLEFT     = 230;     	//LEFT - Forward Right
    //int MTGClicksLeftCENTER    = -609;     	//CENTER - Forward Right
    //int MTGClicksRightCENTER   = 609;     	//CENTER - Forward Right
    //int MTGClicksLeftRIGHT     = -913;     	//RIGHT - Forward Right
    //int MTGClicksRightRIGHT    = 913;     	//RIGHT - Forward Right


    double MoveToWall           = 2.0;        	// was 2.0
    int MoveToWallClicksLeft    = 500;     	//Forward 4 inches 205 273
    ...
    double BackUp               = 0.0;        	//REMOVED TO DRIVE RIGHT INTO BOX
    int BackUpClicksLeft        =  205;      	//Backward
    int BackUpClicksRight       = -205;     	//Backward
    double BackUpPower 		    = .4;

    //double PushGlyph            = 0.0;        	//REMOVED TO DRIVE RIGHT INTO BOX
    //int PushGlyphClicksLeft     = -492;     	//Forward was -600
    //int PushGlyphClicksRight    =  492;       	//Forward was
    //double PushPower 			= .4;

    double BackUpFinal          = 2.0;        	//
    ...
    //Color Arm Sideways
    static final double Servo6Left      = .32; //was .6
    static final double Servo6Right     = .68; //was .4
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            rotateglyph.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE); 	// ADDED BY CHUCK JAN 28
            ...
            telemetry.addData("START AUTO BLUE BACK: ", 0);
            ...
            //
            //START, GRAP GLYPH IN FRONT, RESET COLOR ARM TO TOP POSITION
            //
            StartTime = 0;
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                ...
                telemetry.addData("RESET ARM: ", MAX_POS);
                telemetry.addData("CENTER ARM: ", Servo6Center);
                telemetry.update();
            }
            //
            //DROP COLOR ARM, RAISE LIFT ARM WITH GLYPH
            //
/*
            StartTime = EndTime + .1;
            EndTime = StartTime + DropColorArm;
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Drop Color Arm, Raise Glyph Arm:  ", "2");
                //telemetry.addData("StartTime   ", StartTime);
                //telemetry.addData("EndTime     ", EndTime);
...
*/
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Read Color, Stop Glyph Arm: ", "3");
                //telemetry.addData("StartTime   ", StartTime);
                //telemetry.addData("EndTime     ", EndTime);
                ...
                if (hsvValues[0] >= 130 && hsvValues[0] <= 250) {	//was 150
                    //add variable
                    ColorBlue = 1;
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                ...
                if (ColorBlue == 1) {
                    ...
                    //backleftmotor.setPower(KnockOffSpeedPos);   //Move Back was .2
                    //backrightmotor.setPower(KnockOffSpeedPos);
                    ...
                    //backleftmotor.setPower(KnockOffSpeedNeg);   //Move Forward
                    //backrightmotor.setPower(KnockOffSpeedNeg);
                    // finish red
...
/*            //
            //RESET ROBOT ON THE BOARD
            //
            StartTime = EndTime + .1;
            EndTime = StartTime + ResetMove;

            if(runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Reset Robot: ", "7");

                if (ColorBlue == 1) {
                    //telemetry.addData("Knock Off Ball, Backward TRUE ", "Running: " + runtime.toString());
                    backleftmotor.setPower(ResetSpeedNeg);   	//Move Back was .2
                    backrightmotor.setPower(ResetSpeedNeg);
                    //liftmotor.setPower(1);
                } else {
                    //telemetry.addData("Knock off Ball, Forward FALSE ", "Running: " + runtime.toString());
                    backleftmotor.setPower(ResetSpeedPos);   //Move Forward
                    backrightmotor.setPower(ResetSpeedPos);
                }
            }
*/
            //
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("STOP After Reset: ", "8");
                //telemetry.addData("StartTime   ", StartTime);
                //telemetry.addData("EndTime     ", EndTime);
            ...
            //BACK BLUE, DO NOT MOVE OFF BOARD
            //
            //
            //
            StartTime = EndTime + .1;
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Encoder, Move Off Board: ", "9");
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Stop and Reset: ", "10");
            ...
            //
            //MOVE PARALLEL TO CRYPOBOX
            //
            StartTime = EndTime + .1;
            EndTime = StartTime + MoveOffBoard;

            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Encoder, Move Parallel: ", "13");
                telemetry.addData("BackLeftMotor : ", backleftmotor.getCurrentPosition());
                telemetry.addData("BackRightMotor: ", backrightmotor.getCurrentPosition());
                ...
                frontleftmotor.setPower(MTGPower);
                backleftmotor.setPower(MTGPower);
                frontrightmotor.setPower(MTGPower);
                backrightmotor.setPower(MTGPower);
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                ...
                //REMOVED BECAUSE BLOCK WAS DRAGGING ON THE SPIN
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                ...
                liftmotor.setPower(GlyphMotorPowerDown);
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Stop and Reset, Lower Glyph Arm: ", "16");
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                ...
                servo0.setPosition(Servo0Open);
                servo1.setPosition(Servo1Open);
            }
/* REMOVED
	    //
            //BACK UP AFTER DROP - REMOVED
            //
            StartTime = EndTime + .1;
            EndTime = StartTime + BackUp;

            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Encoder, Backup After Drop: ", "16");
                telemetry.addData("BackLeftMotor : ", backleftmotor.getCurrentPosition());
                telemetry.addData("BackRightMotor: ", backrightmotor.getCurrentPosition());
                ...
                frontleftmotor.setPower(BackUpPower);
                backleftmotor.setPower(BackUpPower);
                frontrightmotor.setPower(BackUpPower);
                backrightmotor.setPower(BackUpPower);

                frontleftmotor.setTargetPosition(BackUpClicksLeft);
                backleftmotor.setTargetPosition(BackUpClicksLeft);
                frontrightmotor.setTargetPosition(BackUpClicksRight);
                backrightmotor.setTargetPosition(BackUpClicksRight);
            }
            //
            //STOP AND RESET, OPEN GLYPH BEFORE PUSH - REMOVED
            //
            StartTime = EndTime + .1;
            EndTime = StartTime + StopEncoders;
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Encoder, Push Glyph: ", "21");
                telemetry.addData("BackLeftMotor : ", backleftmotor.getCurrentPosition());
                telemetry.addData("BackRightMotor: ", backrightmotor.getCurrentPosition());
                ...
                frontleftmotor.setPower(PushGlyphPower);
                backleftmotor.setPower(PushGlyphPower);
                frontrightmotor.setPower(PushGlyphPower);
                backrightmotor.setPower(PushGlyphPower);

                backleftmotor.setTargetPosition(PushGlyphClicksLeft);
                backrightmotor.setTargetPosition(PushGlyphClicksRight);
            }
            //
            //STOP AND RESET - REMOVED
            //
            StartTime = EndTime + .1;
            EndTime = StartTime + StopEncoders;
...
*/
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Encoder, Backup Final: ", "23");
                telemetry.addData("BackLeftMotor : ", backleftmotor.getCurrentPosition());
                telemetry.addData("BackRightMotor: ", backrightmotor.getCurrentPosition());
            ...
            //
            //SPIN 90 DEGREES RIGHT, TO POINT TO PILE
            //
            /*StartTime = EndTime + .1;
            EndTime = StartTime + Spin;

            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("Spin to Pile: ", "25");
                telemetry.addData("BackLeftMotor : ", backleftmotor.getCurrentPosition());
                telemetry.addData("BackRightMotor: ", backrightmotor.getCurrentPosition());
                ...
                frontleftmotor.setTargetPosition(-Spin90left);	//Use Neg to Spin Right
                backleftmotor.setTargetPosition(-Spin90left);	//Use Neg to Spin Right
                frontrightmotor.setTargetPosition(-Spin90right);
                backrightmotor.setTargetPosition(-Spin90right);
            ...
            if (runtime.seconds() >= StartTime && runtime.seconds() <= EndTime) {
                telemetry.addData("FINAL STOP: ", "26");
